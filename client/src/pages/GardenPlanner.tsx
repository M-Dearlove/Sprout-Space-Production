...
const GardenPlanner: React.FC = () => {
  ...
  const handleSearchSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error: refetchError } = await refetch({ searchTerm, limit: 8 });
      
      if (refetchError) {
        console.error('GraphQL error during refetch:', refetchError);
        throw new Error(refetchError.message);
      }
      
      console.log('Search results:', data?.searchPlants);
      
      if (data && data.searchPlants && data.searchPlants.length > 0) {
        const formattedResults = data.searchPlants.map((plant: any) => {
          console.log('Processing plant result:', plant);
          
          // Handle different data structures from DB vs Perenual API
          const plantId = plant._id || plant.id || `plant-${Math.random().toString(36).substr(2, 9)}`;
          const plantName = plant.plantName || plant.common_name || plant.name || "Unknown Plant";
          ...
          return {
            ...
            plantsPerSquareFoot: plant.plantsPerSquareFoot || calculatePlantsPerSquareFoot(plant.spacing || 12),
            sunlight: plant.plantLight || (Array.isArray(plant.sunlight) ? plant.sunlight.join(', ') : plant.sunlight) || 'Unknown',
            water: plant.plantWatering || plant.watering || plant.water || 'Unknown',
            image: plant.plantImage || plant.image || plant.default_image?.thumbnail || 'https://cdn-icons-png.flaticon.com/128/628/628324.png'
          };
        });
        ...
        console.log('No plants found in search results');
        setSearchError('No plants found matching your search.');
        setSearchResults([]);
      }
    } catch (error) {
      ...
      if (error instanceof Error) {
        setSearchError(`Error searching plants: ${error.message || 'Unknown error'}. Please try again.`);
      } else {
        setSearchError('Error searching plants: Unknown error. Please try again.');
      }
    } finally {
      setIsSearching(false);
    }
