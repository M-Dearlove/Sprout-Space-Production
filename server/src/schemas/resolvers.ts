    ...
    searchPlants: async (_parent: any, { searchTerm, limit = 8 }: { searchTerm: string, limit?: number }) => {
      try {
        ...
        if (dbResults.length < limit) {
          ...
          // Check if there was an error from the Perenual API
          if (perenualResults && 'error' in perenualResults) {
            console.error('Perenual API error:', perenualResults.error);
            // Just return DB results if Perenual fails
            return dbResults;
          }
